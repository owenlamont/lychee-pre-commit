...
# ]
# ///
"""Update lychee-pre-commit to mirror the latest lychee-bin release."""
...
def main() -> None:
    """Update this repository to track mirrored lychee-bin releases."""
    pyproject = _load_pyproject()
    current_version = _current_lychee_version(pyproject)
...
def _available_versions() -> list[Version]:
    response = urllib3.request("GET", "https://pypi.org/pypi/lychee-bin/json")
...
def _wait_for_pypi_release(
    ...
    url = f"https://pypi.org/pypi/lychee-bin/{version}/json"
    elapsed = 0
    ...
    while elapsed <= max_wait_seconds:
        ...
        if elapsed == max_wait_seconds:
            break
        print(
            f"Waiting for lychee-bin {version} on PyPI"
            f" ({elapsed}s/{max_wait_seconds}s)..."
        )
        time.sleep(poll_interval_seconds)
        elapsed += poll_interval_seconds

    raise RuntimeError(
        f"Timed out waiting for lychee-bin {version} on PyPI after {max_wait_seconds}s."
    )


def _current_lychee_version(pyproject: dict) -> Version:
    dependencies = pyproject["project"]["dependencies"]
    requirement: Requirement | None = None
    for dependency in dependencies:
        parsed = Requirement(dependency)
        if parsed.name == "lychee-bin":
            requirement = parsed
            break
    if requirement is None:
        raise AssertionError("pyproject.toml must declare a dependency on lychee-bin")

    specifiers = list(requirement.specifier)
    if len(specifiers) != 1 or specifiers[0].operator != "==":
        raise AssertionError(
            f"lychee-bin specifier must pin to exact version: {requirement}"
        )
...
def _update_files(version: Version) -> Sequence[str]:
    files = {
        "pyproject.toml": lambda content: re.sub(
            r'"lychee-bin==[^"]+"', f'"lychee-bin=={version}"', content
        ),
